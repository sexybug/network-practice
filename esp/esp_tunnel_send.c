...
int main(int argc, char **argv)
{
    for (int i = 0; i < 1; ++i)
    {
        ...
        int data_len = ip_packet_get_packet_len(origin_ip_packet);
        uint8_t plain_data[data_len];
        ip_packet_get_packet_bytes(origin_ip_packet, plain_data);

        //不足整分组，需填充
        int data_len2 = data_len;
        if (data_len2 % 16 != 0)
        {
            data_len2 = (data_len & 0xfffffff0) + 16;
        }
        uint8_t plain_data_pad[data_len2];
        memcpy(plain_data_pad, plain_data, data_len);
        memset(plain_data_pad + data_len, 0, data_len2 - data_len);

        //加密
        uint8_t cipher_data[data_len2];
        int out_len = 0;
        sm4_cbc_enc(key, iv, plain_data_pad, data_len2, cipher_data, &out_len);
        esp_packet_set_data(esp, cipher_data, data_len2);
        //完整性校验值
